function y=opt_control_bounded_input()
...
% inequality constraints in terms of control input.  The initial solution
% is selected from infeasible region. Thus, we first release two
% parameters such that a fold point is detected. Then we do branch switch
% to drive d.obj to zero. We further drive d.p3,...,d.p_ncheb to zeros. In
% the final stage, we drive kappa to zero such that the inequality
...
% Integral constraints regarding input
prob1 = coco_add_func(prob1, 'input', @optcont_input, @optcont_input_du,...
    @optcont_input_dudu, odata, 'inequality', 'in', 'uidx', uidx(maps.p_idx));
...
prob1 = coco_add_adjt(prob1, 'obj', @adj_optcont_obj,...
    @adj_optcont_obj_du, data, 'd.obj',...
    'aidx', axidx([opt.xcn_idx; opt.T0_idx; opt.T_idx; opt.p_idx]),...
    'remesh', @adj_obj_remesh);

prob1 = coco_add_adjt(prob1, 'input', 'ncp.input', 'aidx', axidx(opt.p_idx));
...
cont_args{ncheb+3} = 'ncp.input';
cont_args{ncheb+4} = 'in';
...
prob2 = coco_add_func(prob2, 'input', @optcont_input, @optcont_input_du,...
    @optcont_input_dudu, odata, 'inequality', 'in', 'uidx', uidx(maps.p_idx));
...
prob2 = coco_add_adjt(prob2, 'obj', @adj_optcont_obj,...
    @adj_optcont_obj_du, data, 'd.obj',...
    'aidx', axidx([opt.xcn_idx; opt.T0_idx; opt.T_idx; opt.p_idx]),...
    'l0', chart.x, 'tl0', cdata.v(lidx), 'remesh', @adj_obj_remesh);

[chart, lidx] = coco_read_adjoint('input', 'optcont1', lab, 'chart', 'lidx');
prob2 = coco_add_adjt(prob2, 'input', 'ncp.input', 'aidx',...
    axidx(opt.p_idx), 'l0', chart.x, 'tl0', cdata.v(lidx));
...
for k=1:ncheb-2
    ...
    prob3 = coco_add_func(prob3, 'input', @optcont_input, @optcont_input_du,...
    @optcont_input_dudu, odata, 'inequality', 'in', 'uidx', uidx(maps.p_idx));
    ...
    chart = coco_read_adjoint('input', prun, lab, 'chart');
    prob3 = coco_add_adjt(prob3, 'input', 'ncp.input', 'aidx', axidx(opt.p_idx), 'l0', chart.x);
...
prob3 = coco_add_event(prob3, 'opt', 'BP', 'ncp.input', '==', 0);

% computational domain

cont_args = cell(1,ncheb+3);
cont_args{1} = 'ncp.input';
cont_args{2} = 'obj';
for kid=1:k+2
    cont_args{2+kid} = sprintf('p%d',kid);
end
cont_args{ncheb+3} = 'in';


coco(prob3, crun, [], cont_args, [0 10.87]);
...
figure(1)
plot(tp,u,'r--','LineWidth',2); hold on
xlabel('t');
ylabel('u(t)')
% state variable
lab = coco_bd_labs(bd6, 'opt');
sol = coll_read_solution('', crun, lab(1));
x  = sol.xbp;
[t0, x0]  = ode45(@(t,x) optcont(t, x, zeros(nterm,1)), -1:0.1:1, xa);
figure(2)
plot(sol.tbp*tf/2+tf/2,x(:,1),'r--','LineWidth',2); hold on
xlabel('t');
ylabel('\theta(t)')
figure(3)
plot(sol.tbp*tf/2+tf/2,x(:,3),'r-.','LineWidth',2); hold on
xlabel('t');
ylabel('x(t)')
...
function [data, y] = optcont_input(prob, data, u) %#ok<INUSL>
...
function [data, J] = optcont_input_du(prob, data, u) %#ok<INUSD,INUSL>
...
function [data, dJ] = optcont_input_dudu(prob, data, u) %#ok<INUSD,INUSL>
