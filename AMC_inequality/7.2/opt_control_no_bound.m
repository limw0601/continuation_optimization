function y=opt_control_no_bound()

% in this version, we minimize objective funtional combined with both
% output and input. No bounds are imposed to either input or output
...
prob = coco_set(prob, 'cont', 'ItMX', 120); % 120 with obj upper bound 4000
...
prob1 = coco_add_func(prob1, 'output', @optcont_output, @optcont_output_du,...
    data, 'active', 'out', 'uidx', uidx, 'remesh', @output_remesh);
% adjoints
...
for kid=1:ncheb-1
    cont_args{3+kid} = sprintf('d.p%d',kid+1);
end
...
for k=1:ncheb-1
    ...
    data  = out_init_data(data);
    prob3 = coco_add_func(prob3, 'output', @optcont_output, @optcont_output_du, data,...
        'active', 'out', 'uidx', uidx, 'remesh', @output_remesh);
    ...
    'aidx', axidx([opt.xcn_idx; opt.T0_idx; opt.T_idx; opt.p_idx]), 'l0', chart.x,'remesh', @adj_obj_remesh);

    % computational domain
    chart = coco_read_adjoint('coll.pars', prun, lab, 'chart');
    dp10  = chart.x(k+1);
    dmu = sprintf('d.p%d',k+1);
    if dp10>0
      dp1_int = [-0.1 dp10];
...
figure(1)
plot(tp,u,'k-','LineWidth',2); hold on
xlabel('t');
ylabel('u(t)')
% state variable
...
figure(2)
plot(sol.tbp*tf/2+tf/2,x(:,1),'k-','LineWidth',2); hold on
xlabel('t');
ylabel('\theta(t)')
figure(3)
plot(sol.tbp*tf/2+tf/2,x(:,3),'k-','LineWidth',2); hold on
xlabel('t');
ylabel('x(t)')
...
function [data, y] = optcont_obj(~, data, u)
