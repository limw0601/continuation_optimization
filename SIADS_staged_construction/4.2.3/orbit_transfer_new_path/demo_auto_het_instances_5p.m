...
dest_dir = 'FreeEndPointInit_5p';
aprint = strcat(dest_dir,'Log');
diary(aprint);
...
% prob1 = coco_set(prob1, 'cont', 'corrector', 'broydengood');
...
profile on
bd = coco(prob1, 'HALO3', [], cont_args{:}, [2.0 3.2]);
profile viewer
...
for k=klab
    ...
    plot3(sol1.xbp(:,1),sol1.xbp(:,2),sol1.xbp(:,3),'k-','LineWidth',2); hold on
...
prob = coco_prob(); % use this to deal with coexist of autonomous and non-autonomous trajectories,
prob1 = coco_set(prob, 'cont', 'NAdapt', 5, 'h_max', 10);
prob1 = coco_set(prob1, 'cont', 'ItMX', 200);
...
prob1 = ode_coll2coll(prob1, 'connect', 'HALO3', 'connect', lab); % don't use prob = coco_set(prob, 'ode', 'autonomous', false) before this code
[data_connect, uidx_connect] = coco_get_func_data(prob1, 'connect.coll', 'data', 'uidx');
maps_connect = data_connect.coll_seg.maps;
% impose conditions for T0 and T
T_bc_funcs = {@T_bc, @T_bc_du, @T_bc_dudu};
...
pid_ini = [1 11 21 2 12 22]; % released p for initialization
pid_ini = setdiff(pid_ini,22);
pid = zeros(1,24);
pid(1:3:22) = 3:10;
pid(2:3:23) = 13:20;
pid(3:3:24) = 23:30;
...
prob = coco_set(prob, 'cont', 'ItMX', 600);
for k=1:nrun
    ...
    for kid=1:k
        cont_args{13+kid} = sprintf('p%d',pid(kid));
    end
    for kid=k+1:nrun
        cont_args{13+kid} = sprintf('d.p%d',pid(kid));
    end  
...
popt = p(:,idx);
save orbit_10_terms_orbits_v2.mat popt
tf = 2;
...
save connected_orbits_v2.mat sol
x  = sol.xbp;
t  = sol.tbp;
tp = (t+1)*tf/2;
u = zeros(3,numel(t));
...
%% plot family of periodic orbits
for k=1:12
    lab = k;
    sol = coll_read_solution('orbit1', 'Halo1', lab);
    plot3(sol.xbp(:,1),sol.xbp(:,2),sol.xbp(:,3),'b-','LineWidth',2);hold on
end
set(gca,'FontSize', 14);
...
set(h3,'interpreter','Latex','FontSize',18)
% h4 = legend('Empirical network','PT','PTN','PTE','PTA','PTD');
% set(h4,'color','none');
% set(h4, 'Box', 'off');
...
for k=1:8
    lab = k;
    sol = coll_read_solution('orbit2', 'Halo2', lab);
    plot3(sol.xbp(:,1),sol.xbp(:,2),sol.xbp(:,3),'b-');hold on
end

%% plot transfer trajectory without control and glide
sol = coll_read_solution('coll_6', crunid, 2);
plot3(sol.xbp(:,1),sol.xbp(:,2),sol.xbp(:,3),'k.');hold on


%% plot transfer trajectory with control but no glide



%% plot transfer trajectory with control and glide
plot3(x(:,1),x(:,2),x(:,3),'c.');
plot3(x(1,1),x(1,2),x(1,3),'bs','LineWidth',2);
plot3(x(end,1),x(end,2),x(end,3),'bs','LineWidth',2);
...
set(gca,'LineWidth',1.5)
h1 = xlabel('$t$');
h2 = ylabel('Control Inputs');
set(h1,'interpreter','Latex','FontSize',18)
set(h2,'interpreter','Latex','FontSize',18)
h3 = legend('$u_x$','$u_y$','$u_z$');
set(h3,'color','none');
set(h3, 'Box', 'off');
set(h3,'interpreter','Latex','FontSize',18)
